...
Write a function called sortStarWarsCharacters that sorts the characters in the starWarsPeople 
...
Your function should use slice within a loop and return an array of each s
uccessive string result from losing letters to the sharpener, until nothing is left.
...
const howMuchPencil = (str) => {
  ...
  for(let i =1; i < str.length + 1; i++) {
    let work = str.slice(i, str.length);
    result.push(work);
  }
  return result;
};
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(value => {
    ...
    result.push(items.slice(items.indexOf(" ")+1, items.length));
});
  return result;
};
