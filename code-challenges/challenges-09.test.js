...
const addValues = (arr) => {
  ...
  let sum = arr.reduce(function(element, value, idx) {
...
const addPurchases = (arr) => {
  // Solution code here...
  let tot = arr.reduce((elementTow, value) => {
    elementTow = elementTow + value.purchasePrice;
    return elementTow;
  }, 0);
  return tot;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let x = 0;
  let sum = arr.reduce(function(elementTh, value, idx) {
    elementTh = elementTh + value;
    x += 1;
    return elementTh;
  }, 0);
  return x;
...
const returnNames = (arr) => {
  ...
  // Solution code here...
  let arrayThre = arr.reduce((element, item) => {
    return `${element},${item.name}`;
  }, '');
  return arrayThre.split(',').splice(1, arrayThre.length);
};
