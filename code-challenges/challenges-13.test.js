...
Write a function named longestString that takes in an array of 
strings 
...
const firstLetters = (arr) => {
  ...
  let newArray = [];
  arr.forEach(string => {
    newArray.push(string.substr(0, 1))
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let arrayTh = arr.map(n => {
    return n.replace(/\(|\)|\-|\s+/g, '')
  });
  return arrayTh;
  // let newArray = [];
  // arr.forEach(phone => {
  //   newArray.push(`${phone.substr(1, 3)}${phone.substr(6, 8)}${phone.substr(10, 13)}`)
  // })
  // return newArray
};
...
Write a function named onlyOddChars that takes in a
 string and returns only the odd-index characters from that string.
...
along with a target string. Return a Boolean based on whether or not every string in 
the array contains the target string.
------------------------------------------------------------------------------------------------ */
...
const findEvery = (arr, target) => {
  ...
  let x;
  arr.forEach(string => {
    if (string.include(target) === false)
      x = false
  });
  return x
